...
function FeatureItem({ 
  ...
  return (
    <div
      ...
      className={`flex flex-col gap-8 sm:gap-10 md:flex-row md:gap-16 items-center transition-all duration-700 ${
        itemInView
        ...
        <div className="flex items-center gap-3">
          {feature.dotColor && (
            <div className="relative flex items-center justify-center">
              {/* Larger transparent circle behind */}
              <div className={`absolute w-6 h-6 rounded-full ${getDotColorClass(feature.dotColor)} opacity-20`}></div>
              {/* Small dot in front */}
              <div className={`relative w-3 h-3 rounded-full ${getDotColorClass(feature.dotColor)} z-10`}></div>
            </div>
          )}
